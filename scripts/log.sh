...
function log() {
  ...
  if [ "${debug_level}" -gt 0 ] || [ "${severity}" -lt 8 ]; then
  ...
  local -A colours;
  colours['META']='\033[32m'   # Green
  colours['TRACE']='\033[36m'  # Magenta
  colours['DEBUG']='\033[35m'  # Cyan
  ...
  case "${level}" in
    ...
    'debug')
      ...
      ;;
    'trace')
      if [ "${trace_level}" -gt 0 ] && [ "${trace_level}" -gt 0 ]; then
        echo -e "${std_line}";
      fi;
      ;;
    'meta')
...
trap 'prev_cmd=$this_cmd; this_cmd=$BASH_COMMAND' DEBUG \
  ...
  || log error 'DEBUG: FAILED TO ENABLE';
